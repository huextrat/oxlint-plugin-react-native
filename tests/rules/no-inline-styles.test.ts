...
describe('no-inline-styles', () => {
  ...
  describe('invalid', () => {
    ...
    `;
      const out = runOxlintWithPlugin(code, rules);
      expect(out).toContain("react-native(no-inline-styles): Inline style: { backgroundColor: '#FFFFFF', opacity: 0.5 }");
    });
    ...
    `;
      const out = runOxlintWithPlugin(code, rules);
      expect(out).toContain("react-native(no-inline-styles): Inline style: { height: 12 }");
    });
    ...
    `;
      const out = runOxlintWithPlugin(code, rules);
      expect(out).toContain("react-native(no-inline-styles): Inline style: { backgroundColor: '#FFFFFF' }");
    });
  });
});
