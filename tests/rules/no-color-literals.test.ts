...
describe('no-color-literals', () => {
  ...
  describe('invalid', () => {
    ...
    `;
      const out = runOxlintWithPlugin(code, rules);
      expect(out).toContain("react-native(no-color-literals): Color literal: { backgroundColor: '#FFFFFF' }");
    });
    ...
    `;
      const out = runOxlintWithPlugin(code, rules);
      expect(out).toContain("react-native(no-color-literals): Color literal: { backgroundColor: '#ccc' }");
    });
  });
});
